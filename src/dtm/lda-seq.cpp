...
double fit_lda_seq(lda_seq* m, const corpus_seq_t* data,
     ...
     int iter = 0;
     // FLAGS_lda_inference_max_iter = 1;
	...
	// fit the variational distribution
        ...
        {
            if (FLAGS_lda_inference_max_iter == 1)
                FLAGS_lda_inference_max_iter = 2;
            if (FLAGS_lda_inference_max_iter == 2)
                FLAGS_lda_inference_max_iter = 5;
            if (FLAGS_lda_inference_max_iter == 5)
                FLAGS_lda_inference_max_iter = 10;
            if (FLAGS_lda_inference_max_iter == 10)
                FLAGS_lda_inference_max_iter = 20;

            outlog( "\nWARNING: bound went down %17.14f; "
		    "increasing var iter to %d\n",
                    bound-old_bound, FLAGS_lda_inference_max_iter);
        }
